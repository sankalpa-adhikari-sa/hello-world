import { isConditionalFilter } from '../types'
import type {
  ColumnDef,
  ColumnFilter,
  ColumnFiltersState,
...
import type { CrudFilter, CrudOperators } from '../types'
...
export const columnFiltersToCrudFilters = <TData = unknown>({
  ...
  return (
    columnFilters?.map((filter) => {
      ...
      return {
        field: filter.id,
        operator: (operator as CrudOperators) ?? defaultOperator,
        value: filter.value,
      }
    }) ?? []
  )
